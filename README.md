...
assets/
  ...
  kenney-castle-kit/     модульный замок (CC0), измерен, в бандл не едет
...
в `resourceart.html`). Шестой, Kenney Castle Kit, лежит измеренным и принёс
не картинку, а **модель стройки**: стена — замкнутая цепь клеток, деталь
выбирается формой стыка, поворот выводится из обмера, а не подбирается
(§6.1.6, `src/sim/castle.ts`, живой генератор в `castleart.html`).
