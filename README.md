...
assets/
  ...
  kaykit-adventurers/    набор персонажей (CC0), взяты варвар-герой и трое
                         жителей замка: рыцарь, маг и плут
...
| В замке живут трое: рыцарь, маг и плут — ходят, но не торгуют | §6.1.6.1 |
...
Замок при этом перестал быть пустым: по двору ходят трое жителей из набора
KayKit Adventurers — рыцарь, маг и плут (§6.1.6.1, `src/sim/castleFolk.ts`).
**Торговли и заданий у них нет**, и это объявленное состояние: житель ничего
не продаёт и ничего не поручает, тап по нему не делает ничего — ровно как тап
по жителям лагеря. Заодно починился кадр: замер показал, что стена в две клетки
при камере в 30° прячет героя целиком на четверти двора и наполовину — ещё
на трети, поэтому **пока герой во дворе, стены замка гаснут** до прозрачности
0,45. Тень при этом остаётся: стена не исчезла, её видно насквозь.
...
| `?castle`, `?castle=СИД` | замок с жителями — как они ходят, видно сразу |
...
У `?castle` и `?grave` рядом со сценой живёт ручка `камень` в консоли:
`камень.routes()` печатает, кто где стоит, куда идёт и сколько ему до угла
обхода. Без неё сцена показывает кадр, но ответить «ходят ли они и куда»
может только глаз.
