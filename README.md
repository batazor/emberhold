...
а к трате. Что при этом происходит с игроком, видно в «Летописи» §9: карточки
прогресса, три игровые петли и личная хроника открываются из настроек.
...
npm run models     # обмер наборов моделей; -- --write пересобирает *.data.ts
                   # и каталоги; -- --pack=cave — один набор
...
npm run protection # таблица урона, блока и ходов до падения по уровням щита
...
assets/
  ...
  kaykit-dungeon/        полы, стены, лестницы и предметы подземелья (CC0)
  ...
  kenney-cave-kit/       модульная пещера (CC0), проходы, лестницы и ворота
...
| Здоровье — шкала HP; Защита, Блок и уворот действуют послойно | §11.3 |
...
Подземелье собирается двумя модульными слоями: KayKit даёт полы, стены,
колонны и лестницы верхним ярусам, а Kenney Modular Cave Kit — каменные
проходы, многоклеточные залы, настоящие перепады уровня и ворота нижним.
Генератор сначала строит семантический граф без координат: вход, узловой зал,
арена, ориентир, конечная цель, боковой тайник, критический путь и обход.
Только затем его узлы получают непересекающиеся footprints 3×3, 5×3 и 5×5,
а рёбра — парные дверные сокеты и коридоры. Для каждого seed строятся 12
полных вариантов; сломанные отбрасываются, остальные оцениваются по глубине
цели, ходу критического пути, положению ветвей, плотности и близости смысловых
связей к рёбрам Делоне. Содержимое использует отдельный RNG и от выбора формы
не перебрасывается. Направленный A* прокладывает коридоры двумя-тремя понятными
плечами, штрафует частые повороты, длинные прямые, касания комнат и параллельные
ходы. После связной геометрии distance transform и непрерывный шум выедают
только тупиковые ниши: они не создают новых связей и не забирают содержимое.
Непроходимые клетки у прохода заполнены мелкой осыпью, крупные массы 2×2 уходят
глубже в породу, а внешний край получает по четыре кластера на клетку.
Генератор резервирует один подъём на ярусе 2 и два на ярусе 3 до размещения
добычи, врагов и валунов; открытые ворота стоят
в проходах, а закрытые ведут в зарезервированную секретную комнату 3×3.
Пока ключей и отмычек нет, её клетки остаются породой для симуляции.
Проходимостью по-прежнему владеет `blocked`, а модели показывают уже
вычисленную топологию.
...
задаётся локацией. «Летопись» в настройках показывает прогресс и позволяет
скопировать сырые события. Остальное живёт в адресной строке:
...
| `?battle=skeleton\|warrior\|mage\|minotaur\|golem` | открыть вылазку сразу рядом с нужным врагом |
| `?shield` | дать отладочному бою щит максимального уровня |
...
| `?avatars` | матрица SVG-аватаров: все виды, сиды и выпавшие приметы |
