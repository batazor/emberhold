...
assets/
  ...
  kaykit-skeletons/      набор противников (CC0) со скином и клипами, двое взяты в игру
scripts/
...
только про то, что ниже; `sim/` и `core/` не тянут three и DOM; фича видна
другим только через свой `index.ts`; каждая ссылка `§N` в коде существует
...
одно, окружение: камни вылазки и лес вокруг лагеря взяты из набора KayKit
Forest (§6.1.1, каталог в `forestart.html`). Второй набор, KayKit Dungeon,
