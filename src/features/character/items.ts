...
export interface MockItem {
  ...
  /**
   * Модель набора, которой вещь и рисуется — значком в клетке и предметом
   * в руке фигуры (`render/iconView.ts`). Её нет у шлема, куртки, сапог,
   * кольца и настоя: в наборах таких вещей попросту нет, и такая вещь
   * остаётся подписью. Подставлять похожую модель нельзя — похожая
   * читается как «вот эта самая».
   */
