...
export const ITEMS: readonly MockItem[] = [
  ...
  { id: 'щит', name: 'Щит', kind: 'руки', effect: 'Защита +4', cost: 'Занимает обе кисти при кайле' },
  { id: 'сапоги', name: 'Сапоги', kind: 'ноги', effect: 'Шаг дешевле на 10%', cost: 'Шумные: угроза быстрее' },
  { id: 'фонарь', name: 'Рудничный фонарь', kind: 'свободное', effect: 'Обзор +1', cost: 'Занимает свободный слот' },
  { id: 'короб', name: 'Заплечный короб', kind: 'свободное', effect: 'Рюкзак +2', cost: 'В бою не даёт ничего' },
  { id: 'кольцо', name: 'Спокойная рука', kind: 'свободное', effect: 'Под угрозой меньше на 20%', cost: 'Аффикс задан ковкой' },
...
/** Клеток в сумке. Хватает на все вещи макета и ещё на снятое. */
export const BAG_CELLS = 9;
...
export function startPack(): PackState {
  ...
  ['сапоги', 'щит', 'короб', 'кольцо', 'настой'].forEach((id, i) => {
    bag[i] = id;
  });
  return { worn, bag };
}
