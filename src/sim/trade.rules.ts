...
describe('Обмен: прилавок', () => {
   ...
   *
   * Было 7,4 — перемерено 2026-08-24 после расширения генератора замков:
   * новые планы иначе кроят поле, кустам достаётся меньше свободных клеток,
   * и местные сносят на прилавок меньше. Требование, ради которого запас
   * заведён, — «лавка проигрывает добытчику» — держится и на новом числе
   * (0,26 против 2,00 пищи в час, печатает тот же инструмент).
   */
  const LOCALS_FOOD_PER_DAY = 6.12;
  /**
   ...
   * После перемера 2026-08-24 — две сотых: поле новых замков теснее.
   * Отказ от этого не отменяется — он становится редким, а слова ему всё
   * равно положены.
   */
  const EMPTY_SHARE = 0.03;
  ...
  test('§13.8 — на прилавке ровно то, что унесли местные, и это измеримо', () => {
    // Та же выборка, что у `npm run trade`: тест и измеритель не должны
    // расходиться из-за малого набора замков с неслучайным перекосом.
