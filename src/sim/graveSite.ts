...
const DEED: readonly string[] = [
  'Донёс рюкзак до самой эвакуации',
...
function writeEpitaph(rng: Rng, crypt = false): string {
  ...
  if (crypt) return `Склеп ${from}. Здесь те, кто не дошёл до эвакуации`;
...
export function generateGraveSite(seed: number): GraveSite {
  ...
  /**
   * Размер участка. Пятёрка снизу, а не четвёрка, и это поймало правило
   * чтения: на участке 4×4 ряды могил дают две-три клетки, из которых
   * половина уходит в насыпи, — и на всё кладбище оставался один читаемый
   * камень. Мерка записана и проверяется: **не меньше трёх надписей
   * на участок**, иначе это не кладбище, а одна могила с оградой.
   */
  const w = 5 + randInt(rng, 3);
  const d = 5 + randInt(rng, 3);
