...
export type TelemetryEvent =
  ...
  /**
   * §21.4 — покупок за деньги в прототипе нет, и это решение, а не пробел
   * в данных. Труба заведена заранее по прямому требованию того же раздела:
   * «важно лишь, чтобы появление позже не потребовало переделки». У этих
   * двух событий сегодня **нет ни одного зовущего**, и это не недоделка,
   * а весь их смысл: форма записи зафиксирована до магазина, поэтому
   * магазин будет писать в неё, а не изобретать свою.
   *
   ...
   * что именно даёт покупка, назовёт `sku`, когда магазин появится.
   *
  ...
  | {
      ...
      priceMinor: number;
      currency: string;
      /** Где показали. Те же три места, что и у выхода: других экранов нет. */
      where: ExitPoint;
    }
  | { t: 'purchase'; at: number; sku: string; priceMinor: number; currency: string };
