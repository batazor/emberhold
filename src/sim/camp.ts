...
/**
 * §20.3 — стоимость, откалиброванная замерами (`npm run measure`).
 *
 * Метод. Бот-осторожный проходит 300 вылазок на ярус: идёт к ближайшему
 * контейнеру, до которого хватает провианта с учётом дороги назад, иначе —
 * на выход. Замер даёт добычу успешной вылазки, из неё по §11.5 выводится
 * цена: добыча ≈ 70% стоимости следующего улучшения. Раскладка по видам —
 * пропорционально тому, что ярус реально роняет, с ограничениями §13:
 * дерево держит ранние уровни, железо появляется с третьего, кристалл —
 * с пятого.
 *
 * Замер (ночь, пары «ярус — здания» по кривой §16):
 *
 * | ярус | успех | добыча | состав                                  |
 * |------|-------|--------|-----------------------------------------|
 * | 0    | 100%  |  6,3   | соль 4,3 · дерево 1,9                   |
 * | 1    |  80%  |  7,0   | соль 3,6 · дерево 1,9 · железо 1,5      |
 * | 2    |  18%  | 14,6   | соль 5,7 · дерево 2,2 · железо 4,6 · кристалл 2,1 |
 * | 3    |  18%  | 24,4   | соль 8,1 · железо 9,3 · кристалл 6,9    |
 *
 * **Уровни 2–3 стоят на твёрдых данных** — ярусы 0–1 ведут себя как задумано.
 * **Уровни 4–6 предварительные**: на ярусах 2–3 выживает 18% вылазок, и
 * «добыча успешной вылазки» там — выборка коротких везучих забегов, то есть
 * нижняя граница. Как только бой перестанет быть главной причиной провалов
 * (§11.3 хочет 65% провиант / 35% бой, замер даёт 30/70), эти три строки
 * надо пересчитать тем же скриптом.
 *
 * Темп из §20.3: ур. 2 — около полутора вылазок, ур. 3 — две, ур. 4 — три,
 * ур. 5 — пять, ур. 6 — семь (последняя ступень экстраполирована, таблица
 * документа заканчивается на пятом).
 */
export const BUILD_COST: Record<number, Partial<Resources>> = {
  2: { salt: 7, wood: 3 },
  3: { salt: 7, wood: 4, iron: 3 },
  4: { salt: 20, wood: 8, iron: 16 },
  5: { salt: 28, wood: 11, iron: 23, crystal: 10 },
  6: { salt: 57, iron: 65, crystal: 48 },
};
