...
export interface CampState {
  ...
  arrows: number;
  /**
   * §6.1.6 — номер последнего сведённого набега (`siege.ts`). Хранится
   * номером, а не временем: набег считается чистой функцией часов, и номер
   * отвечает на «сколько их было, пока меня не было» вычитанием, без тикающего
   * таймера. Отсутствие поля читается как «первый заход» — старый сейв
   * не должен встречать игрока набегами за всё время, что его не было.
   */
  raidedAt?: number;
