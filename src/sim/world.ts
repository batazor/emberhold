 ...
 * обязана вместить самый людный день — двадцать две вылазки плюс особые
...
/**
 * Сколько особых точек бывает в день. Разброс, а не число: постоянного
...
export function regionAt(day: number): Region {
  ...
  /**
   * Особые точки дня — замок (§6.1.6) и кладбище (§6.1.7). Их несколько,
   * и сколько именно, решает раздача, а не таблица расписания: день,
   * в котором рядом три замка и одно кладбище, обязан быть возможен ровно
   * так же, как день, где рядом только третий ярус.
   *
   ...
   * до них. Клетки берутся следующими из той же перетасованной колоды —
   * значит, ни поверх вылазки, ни друг поверх друга они не сядут никогда.
   *
...
export function worldAt(t: number, visits: readonly Visit[] = []): NodeState[] {
  ...
  return region.nodes.map((_, i) => {
    ...
    /**
     * Событие обещает **вылазке** модификатор (§11.6) — ночь, обвал, чужой
     * след. На прогулочной точке обещать нечего: там нет ни добычи, ни
     * провианта, которых событие касается. Спрашивается поэтому «это
     * вылазка?», а не перечисляются исключения: перечисление уже один раз
     * разошлось с правдой — замок из него выпадал, а кладбище нет.
     */
    const event = node.kind === 'вылазка' ? eventAt(day, node.id, t) : null;
