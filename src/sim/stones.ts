 ...
 * занимают. Тап по валуну — герой подходит и разбивает его за десять
 * замахов; получается один камень. Жест тот же, что у топора и у всего
...
/** Камня за один валун. Ровно как брусок за дерево: единица, а не жила. */
export const MINE_STONE = 1;

/**
 * Замахов на валун. Столько же, сколько на дерево, и это не совпадение:
 * замах один и тот же клип, и цена у него одна. Проверяется требованием
 * «добыча медленнее подбора» на каждом сиде (`stones.rules.ts`) — тем же,
 * из которого выведены десять замахов топора.
 */
export const MINE_SWINGS = 10;
...
export function take(stone: Stone, into: Resources, room = Infinity): number {
  const taken = Math.min(MINE_STONE, room);
  stone.taken = true;
  into.stone += taken;
  return taken;
}
