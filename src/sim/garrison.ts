 ...
 * Кузнеца и охотника (§6.1.13) нет по причине жёстче вкуса: их модели
 * запечены без скелета — ретаргета с рига набора нет, — и ходить им не на
 * чем. Гуляющий без клипа ходьбы скользил бы, а не шёл.
 */
export const DWELLER_LOOKS: readonly DwellerLook[] = ['поселенец'];

/**
 * Кто во дворе стоит, а не ходит. Торговец — по §13.5 (стоящий и есть
 * указатель точки обмена), кузнец и охотник — по устройству модели (выше).
 */
...
function yardWalks(
  ...
  /**
   * Кузнец и охотник (§6.1.13) — стоящие: их модели запечены без скелета,
   * и ходить им не на чем. Стоят они **у края двора**, а не на проходе:
