...
describe('Гарнизон: отряд обходит периметр', () => {
  ...
  test('гарнизон ничего не решает: противников на входе нет', () => {
    // Добыча в замке теперь есть — одна казна (§13.6, `castleSite.rules.ts`), —
    // но гарнизон к ней по-прежнему непричастен: он декорация времени,
    // а стража засады спавнится вылазкой (`springAmbush`), не гарнизоном.
