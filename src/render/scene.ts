 ...
 */
/**
 * Наклон камеры. Экспортируется не ради вида, а ради замера: башня высотой H
 * прячет за собой H · ctg(ELEVATION) клеток земли, и потолок роста башен
 * (§6.1.6) посчитан этим, а не выбран.
 */
export const ELEVATION = (30 * Math.PI) / 180;
const CAM_DIST = 42;
...
export class SceneRig {
  ...
  update(dt: number, heroX: number, heroZ: number, visionRadius: number): void {
    ...
    // Но «темно» ≠ «чёрный экран»: при полах 0.05 и 0.07 замер по буферу
    // (?tier=1&bench=1, luma) давал вне пятна фонаря сплошные нули — рельеф
    // пещеры не читался вовсе. Полы подняты до лунного подсвета 0.2 и 0.16:
    // силуэты за кругом света появляются, а обзор остаётся дальностью
    // фонаря — затухание точечного света не тронуто.
    this.sun.intensity = 0.45 + day * 1.35;
    this.sun.color.setHSL(0.09 + this.night * 0.5, 0.55 - day * 0.25, 0.55 + day * 0.12);
    this.hemi.intensity = 0.3 + day * 0.35;
