 ...
 * замер клипа `Jog_Fwd_Loop` после ретаргета лежит в каталоге UAL (`npm run ual`).
 *
...
/**
 * Во сколько гостевой костёр меньше Кухни: след у него 1×1 против 2×2.
 * Палатка жильца этим числом больше не уменьшается: она и есть Жильё ур. 1,
 * и та же модель вполовину роста читалась не «дальше стоит», а «другая,
 * игрушечная палатка». След в данных остаётся 1×1 (`TENT_FOOT`).
 */
const TENT_LOOK = 0.5;
...
export class CampView {
  ...
  private buildMeadow(): void {
    ...
    this.meadow = new FluffyGrass(terrain, {
      ...
      count: 11000,
      scale: 4,
      // Прижат к земле, как луг поляны: в игровых кадрах трава титульной
      // высоты прятала жильцам ноги. Полный рост остаётся только заставке.
      height: 0.15,
  ...
  rebuildBuildings(): void {
    ...
    for (const id of builtBuildings(this.camp)) {
      ...
      // Середина следа [p, p+2): клетка площадки лежит центром в целых
      // координатах (плитки земли, герой и стены стоят в целых), след
      // занимает мир [p−0.5, p+1.5], и его центр — p+0.5. То же число
      // у стен: клетка стены — две клетки лагеря, деталь стоит в 2s+0.5.
      g.position.set(pos.x + 0.5, 0, pos.z + 0.5);
      this.group.add(g);
      this.buildings.set(id, g);
    }

    // Палатки жильцов (`sim/residents.ts`). Модель и масштаб — то же Жильё
    // первого уровня: палатка и есть Жильё, только чужая, и рисовать её
    // другим мешем или другим ростом значило бы сказать, что это другое
    // жильё. След в данных при этом остаётся 1×1 (`TENT_FOOT`, замер
    // `npm run tents`): модель свешивается за клетку, как крона за свою.
    for (const t of this.camp.tents) {
      const g = this.makeBuilding('hq', 1);
      // След палатки 1×1 — клетка `t` целиком, и её мировой центр — сами
      // целые координаты клетки: +0.5 сдвигал палатку на пол клетки наискось,
      // и жилец, идущий по свободной соседней клетке, задевал её край.
    ...
    this.camp.residents.forEach((r, i) => {
      ...
      // Обе точки лежат **за** моделью, а не в ней: кромка следа Кухни —
      // fire+1.5 (след [p, p+2) в клетках с центром в целых), а у палатки
      // кромка **модели** — tent+1.0: рисуется она в рост Жилья и свешивается
      // за свой след 1×1, и шаг мерится от неё, иначе человек стоит в стене.
      const at = tent === undefined
        ? { x: fire.x + 0.5, z: fire.z + 2.1 }
        : { x: tent.x, z: tent.z + 1.1 };
      ...
      // Точка огня — середина следа Кухни, та же, куда встаёт `FireLight`.
      rig.root.rotation.y = Math.atan2(fire.x + 0.5 - at.x, fire.z + 0.5 - at.z);
  ...
  /** Тап по зданию — для перестановки (§20.4). Зона — мировой след
   *  [p−0.5, p+1.5] с запасом 0.2: раньше она была сдвинута к [p, p+2.2]
   *  и ловила палец на клетку правее-ниже нарисованного. */
  buildingAt(x: number, z: number): BuildingId | null {
    for (const id of builtBuildings(this.camp)) {
      const p = this.camp.layout[id];
      if (x >= p.x - 0.7 && x <= p.x + 1.7 && z >= p.z - 0.7 && z <= p.z + 1.7) return id;
    }
    return null;
  }
