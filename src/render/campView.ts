...
export class CampView {
  ...
  rebuildBuildings(): void {
    ...
    this.camp.residents.forEach((r, i) => {
      ...
      // Обе точки лежат **за** следом, а не в нём: кромка следа Кухни —
      // fire+1.5 (след [p, p+2) в клетках с центром в целых), палатки —
      // tent+0.5, и шаг за кромку держит человека вне модели, а не в стене.
      const at = tent === undefined
        ? { x: fire.x + 0.5, z: fire.z + 2.1 }
        : { x: tent.x, z: tent.z + 0.6 };
