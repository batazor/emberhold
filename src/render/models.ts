...
/**
 * Пламя одно на всю игру: костёр, горн, позже факелы.
 * Мерцание — параметр масштаба, поэтому второй огонь стоит ноль решений.
 */
function flames(cx: number, cy: number, cz: number, sc: number): Piece[] {
  return [
    cone(cx, cy, cz, 0.34 * sc, 0.98 * sc, 7, C.plam),
    cone(cx, cy + 0.08 * sc, cz, 0.18 * sc, 0.7 * sc, 6, C.lat),
  ];
}
...
/** Штаб ур. 2 — бревенчатый дом со знаменем (артбук, 03). */
...
/** Штаб ур. 3 — каменный замок с башней. Вымпел переехал на башню. */
...
/**
 * Кузницы в артбуке нет — она появилась в §14 позже него. Собрана из того же
...
function forgeOpen(): Piece[] {
  ...
  m.push(...flames(-0.75, 0.78, -0.35, 0.55));
...
/** Кузница ур. 2 — каменная, с мехами и трубой. */
