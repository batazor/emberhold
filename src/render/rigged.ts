...
const STATE = {
  ...
  'выстрел': { loop: false },
  // §14.2 — блок держит последний кадр: щит остаётся поднятым, пока боец
  // держит стойку (поле боя, `guarding`). Опустить его сам он не вправе —
  // это показало бы конец блока, которого не было.
  'блок': { loop: false, hold: true },
  ...
  'сидит': { loop: true },
  'встаёт': { loop: false },
  // Работа жильца (§6.1.14): рубка топором и кайление киркой — циклы,
  // как ходьба: жилец работает, пока не прикажут иное, а не один замах.
  // Ими же машет рутина на поляне (§6.1.15).
  'рубит': { loop: true },
  'кайлит': { loop: true },
  // Речь жильца — жест под пузырь реплики (§23.5).
  'разговор': { loop: true },
  // Уворот в бою (§11.3): один уход с линии, дальше покой сам.
  'уклон': { loop: false },
