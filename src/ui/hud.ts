...
export class Hud {
  ...
  /**
   * Пока идёт разговор (знакомство, гость у стен §6.1.6.2, наём лесника
   * §6.1.6.3), низ вылазки уступает место панели разговора — тем же правилом
   * и по той же причине, что приглашению пролога: §6.2.6 держит «низ вылазки —
   * одна панель, а не четыре коробки», а разговор приходит **пятой**,
   * своим слоем внизу экрана, и о нижних кнопках он не знает.
   *
   * Уступают подсказка, кнопки и полка расходников. Полоса состояния сверху
   * остаётся: она не мешает и во время разговора сообщает то же, что всегда.
   */
