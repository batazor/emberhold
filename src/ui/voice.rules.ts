...
// Огород добавил пять событий: приглашение и промах по грядке, нехватку
// посевной пищи, начало роста и остаток таймера. Сбор использует уже
// существующую ресурсную строку и нового голоса не заводит.
const SIZES: Readonly<Record<string, number>> = { подсказка: 11, отказ: 64, событие: 98, заметка: 2, диалог: 39, хроника: 7 };
...
const CENSUS: Readonly<Record<string, Record<Person, number>>> = {
  ...
  // После сводки состояния огорода один прямой приказ остался только у
  // промаха; остальные строки безлично называют цену, фазу или время.
  // Выбор культуры подтверждается без нового приказа: карточка уже объясняет
  // следующий жест, а полоса только фиксирует выбранный посев.
  событие: { вы: 8, ты: 0, я: 0, никто: 90 },
