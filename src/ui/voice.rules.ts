...
const SIZES: Readonly<Record<string, number>> = { подсказка: 11, отказ: 46, событие: 49, заметка: 2, диалог: 26, хроника: 7 };
...
const CENSUS: Readonly<Record<string, Record<Person, number>>> = {
  ...
  //
  // Четыре новых «никто» — сундуки (§13.6): прибавка пролога, отказ места,
  // стража замка и скелеты засады. Ещё одно — уворот в бою (§11.3):
  // «бьёт — уходит от удара». Все — о мире, как положено событию.
  событие: { вы: 6, ты: 0, я: 0, никто: 43 },
