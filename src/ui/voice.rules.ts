...
const TABLES: Readonly<Record<string, Channel>> = {
  ...
  TENT_REASON: 'отказ',
  ENTRY_REASON: 'отказ',
  // Почему отряд не уходит (§25). Причина живёт рядом со своим
  ...
  TALK_TEXT: 'диалог',
  // Строки хроники (`sim/chronicle.ts`, §24). Работа у них событийная —
  ...
  CHRON_TEXT: 'хроника',
  // Отчёт отряда, ушедшего без игрока (`sim/sortie.ts`, §25). Это строка
...
const SIZES: Readonly<Record<string, number>> = { подсказка: 11, отказ: 44, событие: 41, заметка: 2, диалог: 25, хроника: 7 };
...
const CENSUS: Readonly<Record<string, Record<Person, number>>> = {
  ...
  // Три новых отказа — отправки (§25): все безличны, как и остальные сорок
  // один, потому что отказ говорит о мире, а не об игроке.
  отказ: { вы: 0, ты: 0, я: 0, никто: 44 },
  // Четыре строки отчёта отправки (§25) — три исхода и «Отряд ушёл»:
  // о ком угодно, но не к кому-то, как и положено событию.
  событие: { вы: 4, ты: 0, я: 0, никто: 37 },
