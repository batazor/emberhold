...
const TABLES: Readonly<Record<string, Channel>> = {
  ...
  TALK_TEXT: 'диалог',
  // Гость у стен замка (`sim/castleGuest.ts`): откуда он и что ищет —
  // речь человека, тот же канал, что у знакомства.
...
const SIZES: Readonly<Record<string, number>> = { подсказка: 11, отказ: 46, событие: 50, заметка: 2, диалог: 26, хроника: 7 };
...
const CENSUS: Readonly<Record<string, Record<Person, number>>> = {
  ...
  событие: { вы: 6, ты: 0, я: 0, никто: 44 },
