...
//
// Ещё две строки события — кнопка «добыть то, чего не хватает» (§13.3,
// §13.7): кому поставили приказ на пищу и почему ставить некому. Обе
// в канале событий, а не отказов: у задания нет своей таблицы причин —
// оно называет случившееся полосой лагеря.
const SIZES: Readonly<Record<string, number>> = { подсказка: 11, отказ: 65, событие: 92, заметка: 2, диалог: 41, хроника: 7 };
...
const CENSUS: Readonly<Record<string, Record<Person, number>>> = {
  ...
  // Двенадцатое «никто» — наём лесника (§6.1.6.3): «⟨⟩ нанят · монеты −⟨⟩»
  // называет случившееся и цену, ни о чём не прося. Тринадцатое
  // и четырнадцатое — сбор недостающего (§13.3, §13.7): «⟨⟩: добывать пищу»
  // и «Добывать пищу некому — ягоды растут в местах мира». Оба о мире:
  // первое называет, кого послали, второе — где растёт то, чего просят.
  // Пятнадцатое — «Дерево растёт в местах мира»: тот же ответ кадру,
  // в котором рубить нечего, и та же безличность.
  событие: { вы: 7, ты: 0, я: 0, никто: 85 },
