...
export class StatsPanel {
  ...
  private render(): void {
    ...
    const rows: [string, string, string][] = [
      ...
      ['Провалов', pct(s.failRate), 'бой не главная причина — §11.3'],
      // §22.6 — доля считается от провалов: вопрос «из-за чего проигрывают»,
      // а не «часто ли». Целевое соотношение выводится моделью, а не назначено.
      ['Из них в бою', pct(s.combatFailShare), 'цель — §22.6, прежние 35% черновые'],
      ['Ран за вылазку', s.avgWoundsTaken.toFixed(1), 'бой стоит ран, а не времени'],
      ...
      ['Провианта на выходе', s.avgFoodLeft.toFixed(1), 'много остатка — ярус слишком лёгкий'],
