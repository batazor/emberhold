...
export interface WorldMapCallbacks {
  ...
  /** §25 — то же место, но идёт отряд, а игрок остаётся в лагере. */
...
/** Доля отправки словами карточки: то же число, что режет добычу (§25). */
...
export class WorldMap {
  ...
  /**
   * §25 — вторая кнопка карточки. Ниже входа и мельче его намеренно:
  ...
  /**
   * §25 — предложение отправить отряд. Глубже `SORTIE_MAX_TIER` строки нет
