...
/**
 * Слова словаря, которым коробку объявлять можно: три коробки, круг портрета
 * и `button` — он и есть коробка по умолчанию.
 *
 * `.map-cv` — единственное названное исключение, а не слово: это холст карты,
 * набить его отступом нельзя, а край карте нужен (§4.2). Исключение стоит
 * здесь поимённо и не обходится молчанием.
 */
const BOXES = new Set(['.card', '.panel', '.chip', '.face', 'button', '.map-cv']);
...
const FLAT = rules(
  BARE
    .replace(/@media[^{]*\{(?:[^{}]*\{[^{}]*\})*[^{}]*\}/g, '')
    // `from`, `to` и проценты повторяются законно в разных анимациях: это
    // шаги `@keyframes`, а не селекторы основного потока таблицы стилей.
    .replace(/@(?:-webkit-)?keyframes[^{]*\{(?:[^{}]*\{[^{}]*\})*[^{}]*\}/g, ''),
);
