...
if (debugCastle !== null) {
  ...
  (window as unknown as { камень: unknown }).камень = {
     ...
     * запас суточный: `прилавок(+86400)` показывает завтрашний.
     */
    прилавок: (shift = 0) => {
      if (castleNow === null) return null;
      ...
      return {
        принесли: supply,
        выкуплено: (camp.bought ?? {})[marketKey(castleNow.loc.seed, at)] ?? 0,
        на_прилавке: stockOf(supply, camp.bought ?? {}, castleNow.loc.seed, at),
        кустов: castleNow.bushes.length,
      };
    },
