-- ВНИМАНИЕ: это первый файл миграции в репозитории, но не первая миграция
-- в базе. Пять предыдущих (`saves`, `world_visits`, `server_time`,
-- `telemetry_events`, `sortie_and_wheel`) применялись мимо файлов и лежат
-- только в самой базе — `supabase migration list` покажет их, а
-- `supabase db reset` по этому каталогу соберёт **одну** таблицу из шести.
-- Выгрузить остальные — отдельная работа и не этой ветки; до тех пор
-- источник правды по схеме — база, а каталог — только запись о том,
-- что сделала эта ветка.
