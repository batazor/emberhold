...
# Папка ассетов. Считается от самого файла, а не пишется абсолютным путём:
# сборка живёт в ветках и воркtree-копиях, и вписанный путь означал бы, что
# драйвер работает ровно в той папке, где его однажды запустили.
ASSETS = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..") + os.sep
...
def settler_female():
    """Поселенка: длинные волосы, платье с передником, закатанные рукава.

    Силуэт делают юбка-колокол и волосы до лопаток: при камере в 30 градусов
    низ и макушка читаются первыми. Чепца нет — волосы и есть шапка. Грудь —
    две скруглённые формы на кости груди, языком набора: объём даёт форма
    с фаской, а не скульптура. Скелет общий (Rig_Medium): все клипы игры
    водят её как есть, своих костей набор не заводит.
    """
    f = Folk('Settler_Female')
    head(f, 'кожа', 'дерево')
    f.box('head', 'земля-тень', (-0.098, 0.098), (-0.505, -0.450), (1.556, 1.592), bevel=0.016, seg=2)  # рот
    f.box('head', 'дерево', (-0.400, 0.400), (0.240, 0.540), (1.280, 2.060),
          taper=(1.0, 0.9), bevel=0.08, seg=2)                                          # волосы по спине
    # Лиф платья: плечи уже мужского корпуса.
    f.box('chest', 'мох', (-0.350, 0.350), (-0.275, 0.275), (0.900, 1.310), bevel=0.06, seg=2)
    for s_ in (1, -1):                                                                  # грудь
        f.ball('chest', 'мох', (s_ * 0.155, -0.300, 1.115), (0.150, 0.130, 0.155), u=10, v=6)
    f.box('spine', 'мох', (-0.345, 0.345), (-0.270, 0.270), (0.600, 1.060), bevel=0.055, seg=2)
    # Юбка-колокол в две ступени, низ шире плеч: силуэт делает именно она.
    f.box('hips', 'мох', (-0.440, 0.440), (-0.340, 0.340), (0.290, 0.700),
          taper=(0.76, 0.78), bevel=0.08, seg=2)
    f.box('hips', 'мох', (-0.550, 0.550), (-0.420, 0.420), (0.130, 0.360),
          taper=(0.80, 0.82), bevel=0.10, seg=2)
    # Передник поверх юбки спереди — то, чем поселенка отличается от любого
    # силуэта двора с первого взгляда.
    f.box('hips', 'солома', (-0.240, 0.240), (-0.435, -0.360), (0.210, 0.660),
          taper=(0.80, 1.0), bevel=0.04, seg=2)
    f.box('hips', 'земля-тень', (-0.360, 0.360), (-0.300, 0.300), (0.660, 0.745), bevel=0.035, seg=2)  # пояс
    limbs(f, sleeve='мох', cuff='кожа', skin='кожа', trouser='сукно-тень', boot='земля-тень', foot_h=0.140)
    return f
...
def forester():
    """Лесник (§6.1.6.3): хвойный плащ с откинутым капюшоном и колчан за спиной.

    Капюшон **откинут**, а не надет, и это не украшение: при камере в 30°
    надетый капюшон затеняет всё лицо — ровно то, из-за чего его не надели
    поселенцу. Откинутый он делает ту же работу силуэтом, оставляя лицо.

    Отличать его нужно от охотника, а не от поселенца: оба из леса, оба
    в куртке до бёдер. Разводят их цвет и спина — у охотника алое и перо
    на шапке, здесь хвойное и колчан.

    Предмета в кулаке нет намеренно. Топор ему кладёт игра (§6.1.14,
    `RESIDENT_TOOL`), потому что топор здесь — занятие, а не человек:
    переведённый на камень лесник обязан взять кайло, а запечённый в кисть
    топор остался бы с ним навсегда.
    """
    f = Folk('Forester')
    head(f, 'кожа', 'дерево-тень')
    f.box('head', 'земля-тень', (-0.098, 0.098), (-0.505, -0.450), (1.556, 1.592), bevel=0.016, seg=2)  # рот
    # Налобный ремень: у лесника нет ни шапки, ни чепца — волосы держит ремень.
    f.box('head', 'земля-тень', (-0.530, 0.530), (-0.555, 0.505), (1.905, 1.985), bevel=0.03, seg=2)
    torso(f, 'хвоя', hips=False)
    # Плащ на плечах: шире груди и с запасом назад. Он и есть силуэт —
    # сверху, с игровой камеры, видно ровно его.
    f.box('chest', 'хвоя', (-0.470, 0.470), (-0.330, 0.430), (1.040, 1.345),
          taper=(0.92, 0.92), bevel=0.075, seg=2)
    # Откинутый капюшон: горб на спине от лопаток до затылка.
    f.box('chest', 'хвоя', (-0.290, 0.290), (0.230, 0.560), (1.120, 1.520),
          taper=(0.80, 0.75), bevel=0.10, seg=2)
    f.box('hips', 'хвоя', (-0.400, 0.400), (-0.305, 0.305), (0.445, 0.690),
          taper=(0.86, 0.88), bevel=0.075, seg=2)                                      # подол
    f.box('hips', 'земля-тень', (-0.392, 0.392), (-0.298, 0.298), (0.640, 0.735), bevel=0.038, seg=2)  # пояс
    # Колчан за правым плечом и три оперения из него. Оперение мхом — тем же
    # цветом, что перо охотника: перо в этом наборе одного цвета.
    f.box('chest', 'дерево-тень', (0.130, 0.330), (0.330, 0.530), (0.980, 1.430),
          taper=(0.90, 0.90), bevel=0.05, seg=2)
    for dx in (0.0, 0.075, -0.075):
        f.box('chest', 'мох', (0.195 + dx, 0.265 + dx), (0.395, 0.465), (1.400, 1.560),
              taper=(0.7, 0.7), bevel=0.02, seg=2)
    limbs(f, sleeve='хвоя', cuff='земля-тень', skin='кожа', trouser='сукно-тень', boot='земля-тень')
    return f
...
def assemble():
    for ob in list(bpy.data.objects):
        if ob.name in ('Settler', 'Settler_Female', 'Merchant', 'Blacksmith', 'Hunter', 'Forester') or ob.name.endswith('_Rig'):
            bpy.data.objects.remove(ob, do_unlink=True)
    made = []
    for folk, dx in ((settler(), 0.0), (settler_female(), -2.2), (merchant(), 2.2), (blacksmith(), 4.4),
                     (hunter(), 6.6), (forester(), 8.8)):
