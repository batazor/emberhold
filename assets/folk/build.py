...
def assemble():
    for ob in list(bpy.data.objects):
        if ob.name in ('Settler', 'Settler_Female', 'Merchant', 'Blacksmith', 'Hunter') or ob.name.endswith('_Rig'):
            bpy.data.objects.remove(ob, do_unlink=True)
    made = []
    for folk, dx in ((settler(), 0.0), (settler_female(), -2.2), (merchant(), 2.2), (blacksmith(), 4.4), (hunter(), 6.6)):
