...
class Folk:
    ...
    def box(self, bone, m, x, y, z, taper=None, shift=None, bevel=0.0, seg=1):
        ...
        if bevel:
            self._bevel(vs, bevel, seg, si, bi)
        return vs
...
def head(f, skin, hair, brow=True):
    """Голова: скруглённая коробка вместо трёх плиток, плюс лицо.
    ...
    # Оболочки (затылок, чепец) шире головы с запасом, а не впритык. Скруглённый
    # низ коробки уходит внутрь на радиус фаски, и там, где голова всего шире,
    # запас в сотую съедается целиком: наружу выходит кожа, и читается это
    # не щелью, а лысиной.
    f.box('head', hair, (-0.525, 0.525), (0.050, 0.480), (1.470, 2.070), bevel=0.17, seg=2)  # затылок
    for s in (1, -1):
        f.ball('head', skin, (s * 0.495, -0.020, 1.800), (0.060, 0.085, 0.115), u=8, v=5)    # ухо
        f.ball('head', 'мрак', (s * 0.185, -0.480, 1.850), (0.075, 0.058, 0.098), u=8, v=6)  # глаз
        if brow:
            f.box('head', hair, (s * 0.095, s * 0.285), (-0.520, -0.468), (1.945, 1.982),
                  bevel=0.014, seg=1)
    f.ball('head', skin, (0.0, -0.500, 1.775), (0.068, 0.075, 0.082), u=8, v=5)  # нос
...
def torso(f, cloth, hips=True):
    ...
    f.box('chest', cloth, (-0.395, 0.395), (-0.300, 0.300), (0.900, 1.310), bevel=0.10, seg=2)
    f.box('spine', cloth, (-0.385, 0.385), (-0.292, 0.292), (0.600, 1.020), bevel=0.09, seg=2)
    if hips:
...
def limbs(f, sleeve, cuff, skin, trouser, boot, foot_h=0.150):
    ...
    for s in (1, -1):
        ...
        f.capsule('upperarm' + side, sleeve, (s * 0.205, 0.012, 1.123), (s * 0.470, 0.012, 1.123), 0.150)
        f.capsule('lowerarm' + side, cuff, (s * 0.470, 0.012, 1.123), (s * 0.735, 0.012, 1.123), 0.136)
...
def settler():
    ...
    f.box('head', 'земля-тень', (-0.098, 0.098), (-0.512, -0.455), (1.586, 1.622), bevel=0.016, seg=2)  # рот
    f.box('head', 'соль-тень', (-0.560, 0.560), (-0.575, 0.525), (2.005, 2.350),
          taper=(0.89, 0.91), bevel=0.120, seg=3)                                       # чепец, темя
    f.box('head', 'соль-тень', (-0.545, 0.545), (-0.080, 0.520), (1.430, 2.060),
          bevel=0.190, seg=2)                                                           # чепец, затылок
    ...
    limbs(f, sleeve='мох', cuff='кожа', skin='кожа', trouser='сукно-тень', boot='земля-тень',
          foot_h=0.145)
    return f
...
def merchant():
    ...
    f.ball('head', 'соль-свет', (0.0, -0.400, 1.505), (0.265, 0.185, 0.245), u=10, v=6)   # борода
    f.ball('head', 'соль-свет', (0.0, -0.505, 1.700), (0.175, 0.075, 0.070), u=8, v=5)    # усы
...
def assemble():
    for ob in list(bpy.data.objects):
        if ob.name in ('Settler', 'Merchant') or ob.name.endswith('_Rig'):
            bpy.data.objects.remove(ob, do_unlink=True)
    made = []
    for folk, dx in ((settler(), 0.0), (merchant(), 2.2)):
